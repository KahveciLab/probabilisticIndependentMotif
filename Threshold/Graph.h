...
public:
	Graph(int nodeNum, char* fileName, double threshold): NodeSize(nodeNum), adjacencyList(nodeNum, dynamic_bitset<>(nodeNum)){
        adjacencyMatrix = new int*[nodeNum];
        for(int i=0;i<nodeNum;i++)
            adjacencyMatrix[i] = new int[nodeNum]();
        readNetworkFile(fileName, threshold);
	}
...
private:
	void readNetworkFile(char* fileName, double threshold);	
};
